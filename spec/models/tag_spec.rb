...
RSpec.describe Tag, type: :model do
  ...
  describe '.fetch_transactions_for_tags_with_names' do
    ...
    it 'returns all transactions for tags with the provided collection of names' do
      transactions = Tag.fetch_transactions_for_tags_with_names(tag_names, user)

      transactions.each do |trans|
        expect(trans).to be_instance_of(TransactionItem)
        expect(trans.tags.length).to eq(1)
        expect(tag_ids).to include(trans.tags.first.id)
      end
    end

    it 'does not include transactions that should not be included' do
      transactions = Tag.fetch_transactions_for_tags_with_names(tag_names, user)
    ...
    it 'does not fail if tag with given name does not exist' do
      ...
      transactions = Tag.fetch_transactions_for_tags_with_names(all_tag_names, user)
